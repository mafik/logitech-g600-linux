...
int ends_with(const char *haystack, const char *suffix) {
  intptr_t suffix_length = strlen(suffix), haystack_length = strlen(haystack);
...
int main() {
  ...
  while (1) {
    ...
    const char *downCommand = downCommands[scancode],
               *upCommand = upCommands[scancode];
    const char *cmdToRun = (pressed) ? downCommand : upCommand;
    if (!cmdToRun || !strlen(cmdToRun))
      continue;
